...
void Join::joinOn(Client* client)
{
    // flag JOIN <channel>
    try
    {
...
Channel* Join::getChannel(std::string& channelName)
{
    return (server->getChannelList().find(channelName)->second);
}

int Join::isKicked(Client* client)
{
...
void Join::leaveAll(Client* client)
{
    if (client != NULL)
    {
...
void Join::addClient(Client* client)
{
...
void Join::sendMSG(Client* client)
{
    std::string msgBuff;
...
int Join::isAlreadyJoined(Client* client)
{
    if (client != NULL)
    {
        std::map<std::string, Channel*>::iterator find_it;
        std::map<std::string, Channel*>::iterator end_it;
...
int Join::checkClientLevel(Client* client)
{
